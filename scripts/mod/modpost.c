...
/* A pointer field of __this_module: its offset and an assembler expression. */
struct this_module_field {
	unsigned int offset;
	const char *value;
};

#define THIS_MODULE_MAX_FIELDS	4
...
/* The fields of __this_module which are not zero, in offset order. */
					   ...
					   struct this_module_field *fields)
{
	...
	if (mod->has_init)
		fields[nr_fields++] = (struct this_module_field)
			{ MOD_OFF_module_init, MOD_FUNC_PREFIX "init_module" };
#ifdef MOD_OFF_module_exit
	if (mod->has_cleanup)
		fields[nr_fields++] = (struct this_module_field)
			{ MOD_OFF_module_exit, MOD_FUNC_PREFIX "cleanup_module" };
#endif
#ifdef MOD_OFF_module_arch_fixup_start
	fields[nr_fields++] = (struct this_module_field)
		{ MOD_OFF_module_arch_fixup_start, "__start_fixup" };
	fields[nr_fields++] = (struct this_module_field)
		{ MOD_OFF_module_arch_fixup_end, "__stop_fixup" };
#endif
				...
				const char *modname)
{
	...
	asm_skip_to(buf, &pos, MOD_OFF_module_name);
