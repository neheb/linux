...
static int ath79_gpio_register_wifi_descriptors(struct device *dev, const char *label)
{
	struct gpiod_lookup_table *lookup;
	int i;

	/* Create a gpiod lookup using gpiochip-local offsets + 1 for NULL */
	lookup = devm_kzalloc(dev, struct_size(lookup, table, ATH79K_WIFI_DESCS + 1), GFP_KERNEL);

	if (!lookup)
		return -ENOMEM;

	for (i = 0; i < ATH79K_WIFI_DESCS; i++) {
		lookup->table[i] = GPIO_LOOKUP_IDX(label, i, "ath9k", i, GPIO_ACTIVE_HIGH);
	}
